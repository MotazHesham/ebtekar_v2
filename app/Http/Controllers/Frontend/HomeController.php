...
class HomeController extends Controller
{
    public function index()
    {
        ...
        $new_products = Product::where('published',1)->orderBy('created_at','desc')->get()->take(10);
        $home_categories = HomeCategory::with('category.products')->where('published',1)->orderBy('created_at','desc')->get();
        $banners_1 = Banner::where('position',1)->where('published',1)->orderBy('created_at','desc')->get()->take(2);
        $best_selling_products = Product::where('published',1)->orderBy('num_of_sale','desc')->get()->take(10);
        return view('frontend.home', compact('sliders','new_products','home_categories','banners_1','best_selling_products'));
    }
