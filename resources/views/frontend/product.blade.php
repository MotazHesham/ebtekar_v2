                        ...
                        <div class="product-slick no-arrow">
                            @if(json_decode($product->photos) != null)
                                @foreach(json_decode($product->photos) as $key => $photo)
                                    <div><img src="{{ image_asset($photo)}}" alt=""
                            ...
                            <div class="pro-group">
                                ...
                                <ul class="pro-price" id="">
                                    @if($product->discount > 0)
                                        <li id="product-price-for-variant">{{ currency_formatting($product->unit_price)}}</li>
                                        <li><span id="product-price-calc-discount">{{ currency_formatting($product->calc_discount($product->unit_price))}}</span></li>
                                    @else
                                        <li id="product-price-for-variant">{{ currency_formatting($product->unit_price)}}</li>
                                    @endif
                                </ul>
                                            ...
                                            <div class="size-box" id="{{$key}}-{{$choice->attribute}}">
                                                <ul>
                                                    @foreach ($choice->values as $key2 => $value)
                                                        <li data-attribute="{{$key}}-{{$choice->attribute}}" @if($key2 == 0) class="active" @endif  style="width: fit-content;">
                                                            <input style="display: none" type="radio" id="{{ $choice->attribute }}-{{ $value }}" name="attribute_{{ $choice->attribute }}" value="{{ $value }}" @if($key2 == 0) checked @endif>
            ...
            <div class="row">
                <div class="col pr-0">
                    <div class="product-slide-5 product-m no-arrow">
                        @foreach (\App\Models\Product::where('subcategory_id', $product->subcategory_id)->where('id', '!=', $product->id)->where('published', '1')->limit(10)->get() as $key => $related_product)
                            @php
                                $front_image = '';
                                $back_image = '';
                            @endphp
                            @if(json_decode($related_product->photos) != null && json_decode($related_product->photos)[0])
                                @php
                                    $front_image = json_decode($related_product->photos)[0] ?? '';
                                    $back_image = json_decode($related_product->photos)[1] ?? $front_image;
                                @endphp
                            @endif
                                    ...
                                    <div class="product-imgbox">
                                        <div class="product-front">
                                            <a href="{{ route('frontend.product',$related_product->slug) }}">
                                                <img src="{{ image_asset($front_image) }}"
                                                    class="img-fluid" alt="product">
                                            </a>
                                        </div>
                                        <div class="product-back">
                                            <a href="{{ route('frontend.product',$related_product->slug) }}">
                                                <img src="{{ image_asset($back_image) }}"
                                                    class="img-fluid" alt="product">
                                            </a>
                                        </div>
